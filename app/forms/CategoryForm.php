...
class CategoryForm extends Form
{
    ...
    public static function categories_tree($categories = null, $selected = null, $depth = 0)
    {
        if (($categories != null) && is_array($categories)) {
            ...
            foreach ($categories as $c) {
                echo '<li>';
                echo $d_string . '<input type="checkbox" name="categories[]" value="' . $c['pk_i_id'] . '" onclick="javascript:checkCat(\'' . $c['pk_i_id'] . '\', this.checked);" ' . (in_array($c['pk_i_id'],
                        $selected) ? 'checked="checked"' : '') . ' />' . (($depth == 0) ? '<span>' : '') . $c['s_name'] . (($depth == 0) ? '</span>' : '');
    ...
    public static function expiration_days_input_text($category = null)
    {
        parent::generic_input_text('i_expiration_days',
            (isset($category) && isset($category['i_expiration_days'])) ? $category['i_expiration_days'] : '', 3);
    }
    ...
    public static function price_enabled_for_category($category = null)
    {
        parent::generic_input_checkbox('b_price_enabled', '1',
            (isset($category) && isset($category['b_price_enabled']) && $category['b_price_enabled'] == 1));
    }
    ...
    public static function multilanguage_name_description($locales, $category = null)
    {
        ...
        foreach ($locales as $locale) {
            ...
            $contentTemp .= '<div class="FormElement"><label>' . __('Name') . '</label><input id="' . $name . '" type="text" name="' . $name . '" value="' . osc_esc_html(htmlentities($value,
